...
- (void)drawGrid:(CGContextRef)context gridRows:(NSUInteger)gridRows gridLineColor:(UIColor *)gridLineColor gridColums:(NSUInteger)gridColums {
...
volFormatter:(NSString *)volFormatter{
if (self.scaleY == 0) return; // 防护
CGFloat rowSpace = self.chartRect.size.height / (CGFloat)gridRows;
CGFloat rightPadding = 6.0; // 右侧间距，可调整
for (int i = 0; i <= gridRows; i++) {
// 计算像素到值
CGFloat position = (CGFloat)(gridRows - i) * rowSpace;
...
CGRect rect = [valueStr getRectWithFontSize:ChartStyle_reightTextSize];
    // 纵向：以刻度对应的 y 为中心，垂直居中显示文本
   CGFloat centerY = [self getY:value];
    CGFloat textY = centerY - rect.size.height / 2.0;
 
    // 防止超出 chartRect 上下边界
    CGFloat minY = CGRectGetMinY(self.chartRect);
    CGFloat maxY = CGRectGetMaxY(self.chartRect) - rect.size.height;
    if (textY < minY) textY = minY;
    if (textY > maxY) textY = maxY;

    // 横向：右对齐并留出 rightPadding，考虑 chartRect.origin.x
    CGFloat textX = CGRectGetMinX(self.chartRect) + CGRectGetWidth(self.chartRect) - rect.size.width - rightPadding;

    [self drawText:valueStr atPoint:CGPointMake(textX, textY) fontSize:ChartStyle_reightTextSize textColor:ChartColors_reightTextColor];
}
}
