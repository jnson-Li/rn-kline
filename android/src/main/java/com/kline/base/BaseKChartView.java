...
public abstract class BaseKChartView extends ScrollAndScaleView {
            ...
            if (showPriceLine) {
                if (yLabelModel == Status.LABEL_WITH_GRID) {
                    ...
                    if (showRightDotPriceLine) {
                        textLeft = tempRight - textWidth - yLabelMarginBorder - 200;
    ...
    protected void renderRightPriceLabel(Canvas canvas, float y, String priceString,
        ...
        float padding = 8f;        // 内边距
        float cornerRadius = 12f;   // 圆角半径

                        // 用 RectF 而不是 Rect（支持浮点）
        RectF rect = new RectF(
                                textLeft - padding,
                                top - padding,
                                textLeft + textWidth + padding,
                                y + halfTextHeight + padding
                                );

                        // 背景矩形（带圆角）
        rightPriceBoxPaint.setStyle(Paint.Style.FILL);
        canvas.drawRoundRect(rect, cornerRadius, cornerRadius, rightPriceBoxPaint);

                        // 边框
        Paint borderPaint = new Paint(priceLineRightTextPaint);
    ...
    public void changeTranslated(float translateX) {
        if (translateX < getMinTranslate() - 500) {
            translateX = getMinTranslate() - 500;
...
//          1 判断数据个数化,更新最后一个数据还是添加数据
            ...
            if (currentCount == 0) { //原没有数据
                ...
                BaseKChartView.this.points = points;
                lastChange();
                if (dataAdapter.getResetShowPosition() && isResetChange == 0) {
                    changeTranslated(getMinTranslate() - 200);
                    isResetChange = 1;
                }
            } else {  //添加数据
                BaseKChartView.this.points = points;
